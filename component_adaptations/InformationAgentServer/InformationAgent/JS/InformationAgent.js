...
function InformationAgent(){
	...
	this.setCircleList				= function(divID)
	{
		...
		$.post("../PHP/getCircles.php", {str : storageLocation + "inCircles/"})
			 .done(function( data ) 
			 {
				var dat = JSON.parse(data);
	...
	function newDataHandler(evt){
		console.log("kind" + evt.detail.kind);
				switch (evt.detail.kind) {
			case "audio":
				newSnapSoundHandler(evt);
				console.log("received audio: feedloop");
				document.getElementById("audioplayer").src = "../USERdataFOLDERwithAverySECRETname/" + storageLocation + "snapsounds/uploaded_audio.wav";
				updateMood(evt);
				break;
			case "ip":
				break;
		...
		$.post("../PHP/getSnapShots.php", {str : storageLocation + "snapshots/"})
			 .done(function( data ) {
				 ...
				 for (var i=0; i<24; i++){	//last 24 images	
					(dat[dat.length -24 + i]!=null)?snapShotBrowser.childNodes[i].src = dat[dat.length -24 + i]:0;
					};
				}
			);
		};
